...
<body>
    <div class="container">
        ...
        @php
        $transactionId = '';
        if (!empty($lastTxn->description) && preg_match('/(#.*?\))/', $lastTxn->description, $matches)) {
        // $matches[1] will contain "#pay_RQI5uceEh3eZYd)"
        $transactionId = rtrim($matches[1], ')'); // remove closing parenthesis only
        }
        @endphp
